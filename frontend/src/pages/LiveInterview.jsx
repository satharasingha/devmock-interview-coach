...
export default function LiveInterview() {
  ...
  const saveInterviewResult = async (finalScore, finalFeedback, questionsAnswers, duration, passed) => {
    try {
      ...
      const response = await fetch("http://localhost:3000/api/auth/interview/save", {
        ...
        body: JSON.stringify({
          ...
          questionsAnswered: questionsAnswers,
          duration: duration,
          passed: passed,
        }),
      });
  ...
  const endSession = async () => {
    ...
    if (allAnswers.length > 0 && !sessionCompleted) {
      ...
      const overallFeedback = {
        strengths: allAnswers.flatMap(a => a.strengths || []).slice(0, 3),
        improvements: allAnswers.flatMap(a => a.improvements || []).slice(0, 3),
      };
  ...
  /* Submit Answer - Now calls backend API and saves answer */
  const submitAnswer = async () => {
    ...
    try {
      ...
      // Store this answer for session summary
      setAllAnswers(prev => [...prev, {
        ...
        timestamp: formatTime(sessionStats.duration),
      }]);
